...
public class _5_adminSystem {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    _5_adminSystem window = new _5_adminSystem();
    ...
    private void initialize() {
        ...
        JLabel lblRoomReport = new JLabel("SYSTEM REPORT");
        ...
        JButton roomBtn = new JButton("Room Reports");
        roomBtn.setFont(new Font("Tahoma", Font.BOLD, 15));
        roomBtn.setBounds(1142, 29, 180, 30);
        frame.getContentPane().add(roomBtn);
        
        roomBtn.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent e) {
                _5_adminRoom faqsFrame = new _5_adminRoom(); 
        ...
        btnRemoveRow.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedRow >= 0) {
                    ...
                    String email = (String) tableModel.getValueAt(selectedRow, 2); // Assuming the email is in column 2

                    String url = "jdbc:mysql://localhost:3306/SuiteSpotDB"; // Replace with your DB details
                    String user = "root"; // Replace with your DB username
                    String password = ""; // Replace with your DB password

                    String deleteQuery = "DELETE FROM contact WHERE email = ?"; // Changed to 'contact' table

                    try (Connection connection = DriverManager.getConnection(url, user, password);
                         java.sql.PreparedStatement preparedStatement = connection.prepareStatement(deleteQuery)) {

                        preparedStatement.setString(1, email); // Set the email for deletion
  ...
  /*  // Method to add a new row to the table
    ...
    private void loadContactData() {
        ...
        String query = "SELECT name, issue, email FROM contact"; // Changed to 'contact' table
        ...
        try (Connection connection = DriverManager.getConnection(url, user, password);
            ...
            while (resultSet.next()) {
                ...
                String description = resultSet.getString("issue");
                String email = resultSet.getString("email");

                tableModel.addRow(new Object[]{name, description, email}); // Added columns based on the contact table
            }
